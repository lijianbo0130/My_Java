...
public class SimHash_New1 {
    ...
    public SimHash_New1() throws IOException {
    	// 空函数
    }
 
    public SimHash_New1(String tokens) throws IOException {
        this.tokens = tokens;
        this.intSimHash = this.simHash();
    }
 
    public SimHash_New1(String tokens, int hashbits) throws IOException {
        this.tokens = tokens;
    ...
    public int hammingDistance(SimHash_New1 other) {
    ...
    public List subByDistance(SimHash_New1 simHash, int distance) {
        // 分成几组来检查
    ...
    private LinkedList<String> readTxt(String fileDir) {

		LinkedList<String> list = new LinkedList<String>();// 写入的数组
		BufferedReader br = null;
		try {
			// 创建FileReader
			FileReader fr = new FileReader(fileDir);
			// 根据FileReader 创建BufferedReader 可以按行读取
    ...
    public static void main(String[] args) throws IOException {
    	...
    	SimHash_New1 sh= new SimHash_New1();
    	LinkedList<String> list=sh.readTxt("src/simHash/1.txt");
        String s = list.get(0);
        System.out.println(s);
        SimHash_New1 hash1 = new SimHash_New1(s, 64);
//        System.out.println(hash1.intSimHash + "  " + hash1.intSimHash.bitLength());
        // 计算 海明距离 在 3 以内的各块签名的 hash 值
//        hash1.subByDistance(hash1, 3);
 
        
        s += "人民群主好的为啥我的郁闷淡定寂寞醉了疯了要死了原理上这次差异有多大呢3相当于伪随机数产生算法";
        SimHash_New1 hash2 = new SimHash_New1(s, 64);
...
//        hash1.subByDistance(hash2, 3);
        ...
        SimHash_New1 hash3 = new SimHash_New1(s, 64);
        ...
        System.out.println(hash1.hammingDistance(hash2) + " " + dis+"	1-2");
        // 根据鸽巢原理（也成抽屉原理，见组合数学），如果两个签名的海明距离在 3 以内，它们必有一块签名subByDistance()完全相同。
        int dis2 = hash1.getDistance(hash1.strSimHash, hash3.strSimHash);
        System.out.println(hash1.hammingDistance(hash3) + " " + dis2+"	1-3");
    }
}