...
class AnalyzeContext {
	...
	/**
	 * 用途说明：我自己定义的一个函数把数词和后面的量词合成一个词
	 */
	void ljbCombineNumWords(boolean useSmart) {
		if (useSmart) {// 智能分词不用改
			return;
		} else {// 非智能分词把数词量词结合
			// 数词集合
			Collection<Lexeme> numList=new LinkedList<Lexeme>();
			// 量词集合
			Collection<Lexeme> quantityList=new LinkedList<Lexeme>();
			// 遍历
			for (Lexeme lexeme:this.results){
				// 把1-9这种数字加入
				if(lexeme.getLexemeType()==Lexeme.TYPE_ARABIC){
					numList.add(lexeme);
				}
				// 把quantifier.dic 里面的词加入
				if(lexeme.getLexemeType()==Lexeme.TYPE_COUNT){
					quantityList.add(lexeme);
				}				
				
				//词元的起始位移
//				System.out.println("词元的起始位移	"+lexeme.getOffset());
//				System.out.println("词元在文本中的起始位置	"+lexeme.getBeginPosition());
//				System.out.println("词元在文本中的结束位置	"+lexeme.getEndPosition());
//				System.out.println("文本内容	"+lexeme.getLexemeText());
//				System.out.println("文本类型int	"+lexeme.getLexemeType());
//				System.out.println("文本类型String	"+lexeme.getLexemeTypeString());
//				System.out.println();
//				
//			    //词元的相对起始位置
//			    private int begin;
...
//			    private int lexemeType;
			...
			// 遍历数词集合 如果数词的结尾等于量词的开头就加入
			for (Lexeme nLexeme:numList){
				// 遍历量词集合
//				System.out.println( ""+nLexeme.getBeginPosition()+nLexeme.getEndPosition());
				for (Lexeme qLexeme:quantityList){
					if (nLexeme.getEndPosition()==qLexeme.getBeginPosition()){
						results.add(new Lexeme(nLexeme.getOffset(), nLexeme.getBegin(), nLexeme.getLength()+qLexeme.getLength(), 48));
//						System.out.println( ""+nLexeme.getBeginPosition()+nLexeme.getEndPosition());
					}
				}
			}
